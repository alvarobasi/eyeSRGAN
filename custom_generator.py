...
import threading
import os
from utils import preprocess_HR, preprocess_LR
...
class DataGenerator(Sequence):
    ...
    def __init__(self,
        ...
        self.hr_shape = self.target_shape + (3,)

        self.lr_shape = (self.target_shape[0] // self.downscale_factor,
                         self.target_shape[1] // self.downscale_factor) + (3,)

        self.num_samples = _count_valid_files_in_directory(self.path, allowed_formats)
        print("{} images were found!".format(self.num_samples))
    ...
    def __getitem__(self, index):
        ...
        with self.lock:
            ...
            list_batches = [os.path.join(self.path, self.filename_list[k]) for k in indexes]

        batch_hr = np.zeros((self.batch_size,) + self.hr_shape, dtype=np.float32)
        batch_lr = np.zeros((self.batch_size,) + self.lr_shape, dtype=np.float32)

        for j, file_name in enumerate(list_batches):
            batch_lr[j], batch_hr[j] = load_img(file_name,
    ...
    def next(self):
        ...
        lr_images, hr_images = self.__getitem__(self.n)
        # Batch index
        self.n += 1
