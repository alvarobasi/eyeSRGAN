...
def img_upscale(file_name):
    max_height = 84
    max_width = 388

    img = cv2.imread(file_name, cv2.IMREAD_COLOR)
    lr_img = cv2.resize(img, (max_width // 4, max_height // 4), interpolation=cv2.INTER_CUBIC)
    #hr_img = cv2.resize(lr_img, (max_width, max_height), interpolation=cv2.INTER_CUBIC)

    base_name = os.path.splitext(file_name)[0]
    output_path = f'{base_name}_lowres.png'
    cv2.imwrite(output_path, lr_img)


def rm_images(file_name):

    base_name = os.path.splitext(file_name)[0]

    if base_name.__contains__('low'):
        os.remove(file_name)
...
if __name__ == "__main__":
    ...
    # output_path = 'E:\\TFM\\'
    list_file_path = './listado_imagenes.npy'
    ...
    pool.map(img_upscale, list_files)
    pool.close()
    pool.join()
